...
def _write_config(
    ...
    The data body is produced with a real TOML serializer (``tomli_w``)
    so that user-derived values (``dialect``, ``source``, table names)
    cannot inject arbitrary configuration via crafted strings.
    """
    ...
    snapshot_path_str = ".dbsprout/snapshots/" if schema.tables else ""
    ...
    header = "# DBSprout configuration — generated by `dbsprout init`\n\n"
    report_hint = (
        "\n# HTML report destination for `dbsprout report` and\n"
        "# `dbsprout generate --report` (uncomment to customize):\n"
        '# [report]\n# output = "./seeds/report.html"\n'
    )
    toml_content = header + tomli_w.dumps(config) + report_hint

    if table_names:
        toml_content += "\n# Per-table overrides (uncomment and customize):\n"
        for name in table_names:
            # ``tomli_w`` is used for the active config above; these are
            # inert comments, but still quote names with TOML special
            # characters so the suggestion is copy-paste correct.
